...
SESSIONS_STORED = 8  # TODO: add to trezorlib?
...
def test_multiple_sessions(client):
    ...
    for i in range(SESSIONS_STORED - 1, 0, -1):
        _init_session(client, session_id=session_ids[i])
        assert _get_xpub(client, passphrase=None) == xpubs[i]

    # creating one more will exceed the limit and therefore the least recently used
    # will get erased
    _init_session(client)
    _get_xpub(client, passphrase="XX")

    # the LRU item is the session_ids[0], it should have been removed
    # -> must ask for passphrase
    _init_session(client, session_id=session_ids[0])
